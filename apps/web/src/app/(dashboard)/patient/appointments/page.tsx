...
import {
    ...
    Download, CalendarPlus, Video, MapPin, Clock, ChevronRight,
...
export default function AppointmentsPage() {
    ...
    const { data: appointments, isLoading } = trpc.medical.listAppointments.useQuery();
        ...
        return (
            <div className="flex h-[60vh] items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-[12px] font-bold text-[#b0b0b0] uppercase tracking-widest">Checking Schedule...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="space-y-12 max-w-[1400px] mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div className="space-y-2">
                    ...
                    <p className="text-[15px] font-medium text-[#8a8a8a] max-w-sm">Manage your upcoming consultations and review past medical visits.</p>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    >
                        <CalendarPlus className="h-5 w-5" strokeWidth={3} />
                        BOOK VISIT
                    </button>
                </div>
            </div>
                        ...
                        <table className="min-w-full divide-y divide-black/[0.04]">
                            <thead>
                                <tr className="bg-black/[0.01]">
                                    <th className="px-8 py-5 text-left text-[11px] font-black uppercase tracking-[0.2em] text-[#b0b0b0]">Doctor</th>
                                    <th className="px-8 py-5 text-left text-[11px] font-black uppercase tracking-[0.2em] text-[#b0b0b0]">Mode & Time</th>
                                    ...
                                    <tr key={i} className="group transition-all hover:bg-black/[0.01]">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="flex flex-col min-w-0">
                                                    <span className="text-[15px] font-black text-[#05050a] truncate tracking-tight">Dr. Sarah Connor</span>
                                                    <div className="flex items-center gap-2 text-[12px] font-medium text-[#8a8a8a]">
                                                        <span>Cardiology</span>
                                                        <span className="h-1 w-1 rounded-full bg-[#d0d0d0]" />
                                                        <span>Princeton-Plainsboro</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col gap-1.5">
                                                <div className="flex items-center gap-2">
                                                    <Badge className="bg-[#b8ff00]/10 text-[#05050a] border-none rounded-lg text-[9px] font-black tracking-widest px-1.5 py-0.5">
                                                        {i % 2 === 0 ? <Video className="h-3 w-3 mr-1 inline" /> : <MapPin className="h-3 w-3 mr-1 inline" />}
                                                        {i % 2 === 0 ? "ONLINE" : "IN-CLINIC"}
                                                    </Badge>
                                                </div>
                                                <div className="flex items-center gap-1.5 text-[12px] font-bold text-[#05050a]">
                                        ...
                                        <td className="px-8 py-6">
                                            <Badge className={cn(
                                                ...
                                                {i === 1 ? <CheckCircle2 className="h-3 w-3 mr-1.5 inline" /> : <Clock className="h-3 w-3 mr-1.5 inline" />}
                                                {i === 1 ? "Confirmed" : "Upcoming"}
                                            </Badge>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                 <button className="h-10 px-4 bg-[#05050a] text-white rounded-xl text-[12px] font-black hover:scale-105 transition-all">
                                                    JOIN CALL
                                                 </button>
            ...
            {filteredAppointments.length === 0 && (
                <div className="flex flex-col items-center justify-center py-32 space-y-6">
                    <div className="h-24 w-24 rounded-[32px] bg-black/5 flex items-center justify-center group">
                        <Calendar className="h-10 w-10 text-[#d0d0d0] group-hover:scale-110 transition-transform" />
                    </div>
                    <div className="text-center space-y-1">
                        <h3 className="text-[20px] font-black text-[#05050a]">No appointments on the horizon.</h3>
                        <p className="text-[14px] font-medium text-[#8a8a8a] max-w-sm">Schedule your first consultation to begin your wellness journey.</p>
                    </div>
                    <button className="bg-[#05050a] text-[#b8ff00] px-8 py-4 rounded-2xl text-[15px] font-black shadow-xl shadow-black/10 hover:scale-[1.05] active:scale-95 transition-all">
                        EXPLORE DOCTORS
                    </button>
                </div>
            )}
